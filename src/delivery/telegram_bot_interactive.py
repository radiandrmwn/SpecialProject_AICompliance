...
class InteractiveTelegramBot:
    ...
    def __init__(self):
        ...
        print("🤖 Interactive PPE-Watch Bot initialized")
        print(f"📱 Monitoring for commands...")
    ...
    def download_file(self, file_id: str, save_path: Path) -> bool:
        ...
        try:
            ...
            if file_size > 20 * 1024 * 1024:
                print(f"⚠️ File too large: {file_size / (1024*1024):.1f}MB (max 20MB)")
                return False
            ...
            print(f"✅ Downloaded file: {save_path} ({file_size / 1024:.1f}KB)")
            return True
    ...
    def handle_start(self, chat_id: int, user_name: str):
        ...
        message = (
            ...
            "• I'll detect people without helmets\n"
        ...
        print(f"✅ Sent welcome to {user_name} (Chat ID: {chat_id})")
    ...
    def handle_help(self, chat_id: int):
        ...
        self.send_message(chat_id, message)
        print(f"✅ Sent help to Chat ID: {chat_id}")
    ...
    def handle_status(self, chat_id: int):
        ...
        self.send_message(chat_id, message)
        print(f"✅ Sent status to Chat ID: {chat_id}")
    ...
    def handle_report(self, chat_id: int, date_arg: Optional[str] = None):
        ...
        if not pdf_path.exists() and not charts_path.exists():
            ...
            print(f"⚠️ No report found for {date_str}")
            return
        ...
        print(f"📤 Sending report for {date_str} to Chat ID: {chat_id}")
        ...
        if success:
            print(f"✅ Report sent for {date_str}")
        else:
            self.send_message(
                chat_id,
    ...
    def handle_latest(self, chat_id: int):
        ...
        print(f"📤 Sending latest report ({date_str}) to Chat ID: {chat_id}")
    ...
    def handle_video(self, chat_id: int, video_data: dict, user_name: str):
        ...
        try:
            ...
            message = (
                ...
                f"*Violation Results:*\n"
                f"• Unique violators \\(no helmet\\): `{unique_violators}`\n"
                f"• Compliant persons \\(with helmet\\): `{compliant}`\n"
                f"• Total detections: {total_violations}\n\n"
            )

            if unique_violators > 0:
                message += f"⚠️ *{unique_violators} person\\(s\\) detected without helmet*\n\n"
            else:
            ...
            if annotated_video.exists():
                print(f"   📤 Sending annotated video...")
            ...
            try:
                ...
                print(f"   📊 Regenerating daily report for {today}...")
                ...
                if stats:
                    ...
                    print(f"   ✅ Report updated: {pdf_path}")
                    ...
                    print(f"   ℹ️ No aggregated stats available yet")

            except FileNotFoundError:
                print(f"   ℹ️ Events file not found yet (will be created on next event)")
            except Exception as e:
    ...
    def run(self):
        ...
        print("\n" + "=" * 70)
        print("🤖 PPE-Watch Interactive Bot Started")
        print("=" * 70)
        print("\n💡 Bot is now listening for commands...")
        print("📱 Users can send commands like /start, /help, /report")
